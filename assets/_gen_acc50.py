...
def gl_round():
    img = new()
    # 렌즈/안경테 1px씩 키움: 렌즈 cx-3..cx+3 → cx-4..cx+4, 외곽 vline 1 더 밖으로
    for cx in [9, 22]:
        rect(img, cx-4, 13, cx+4, 19, (150,200,240,180))
        hline(img, cx-4, cx+4, 12, (40,40,50,255))
        hline(img, cx-4, cx+4, 20, (40,40,50,255))
        vline(img, cx-5, 13, 19, (40,40,50,255))
        vline(img, cx+5, 13, 19, (40,40,50,255))
    hline(img, 13, 18, 16, (40,40,50,255))
    return img

def gl_square():
    img = new()
    # 좌우 1px씩 키움: x=3 → 2, x+10 → x+12 (전체 +2 너비)
    for x in [2, 16]:
        rect(img, x, 12, x+12, 20, (150,200,240,180))
        rect(img, x, 12, x+12, 12, (40,40,50,255))
        rect(img, x, 20, x+12, 20, (40,40,50,255))
        vline(img, x, 12, 20, (40,40,50,255))
        vline(img, x+12, 12, 20, (40,40,50,255))
    hline(img, 13, 18, 16, (40,40,50,255))
    return img
...
def gl_sun():
    img = new()
    for x in [2, 16]:
        rect(img, x, 12, x+12, 20, (40,40,50,255))
    hline(img, 13, 18, 16, (40,40,50,255))
    return img

def gl_monocle():
    img = new()
    rect(img, 16, 13, 24, 20, (150,200,240,180))
    hline(img, 16, 24, 12, (40,40,50,255))
    hline(img, 16, 24, 21, (40,40,50,255))
    vline(img, 15, 13, 20, (40,40,50,255))
    vline(img, 25, 13, 20, (40,40,50,255))
    vline(img, 13, 16, 24, (245,210,80,255))  # chain
    return img
...
def gl_heart():
    ...
    for cx in [9, 22]:
        ...
        # 위쪽 하트 봉오리
        px(img, cx-2, 13, (255,100,140,255)); px(img, cx+2, 13, (255,100,140,255))
        px(img, cx-1, 13, (255,100,140,255)); px(img, cx+1, 13, (255,100,140,255))
    return img

def gl_star():
    img = new()
    for cx in [9, 22]:
        # 5-point simplified — 1px씩 키움
        rect(img, cx-2, 14, cx+2, 17, (245,210,80,255))
        px(img, cx-3, 15, (245,210,80,255)); px(img, cx+3, 15, (245,210,80,255))
        px(img, cx-3, 16, (245,210,80,255)); px(img, cx+3, 16, (245,210,80,255))
        px(img, cx, 13, (245,210,80,255))
    return img

def gl_mask():
    img = new()
    rect(img, 6, 16, 25, 24, (255,255,255,255))
    rect(img, 6, 16, 25, 17, (200,200,210,255))
    return img

def gl_eyepatch():
    img = new()
    rect(img, 5, 12, 15, 21, (40,40,50,255))
    vline(img, 15, 12, 21, (40,40,50,255))
    rect(img, 15, 16, 28, 17, (40,40,50,255))  # strap
    return img
...
def feet_socks():
    img = new()
    for x in [8, 22]:
        rect(img, x-2, 22, x+2, 28, (255,255,255,255))
        rect(img, x-2, 22, x+2, 23, (255,80,80,255))
    return img

def feet_boots():
    img = new()
    for x in [8, 22]:
        rect(img, x-2, 20, x+2, 28, (140,90,60,255))
        rect(img, x-2, 26, x+2, 28, (60,40,30,255))
    return img

def feet_sneakers():
    img = new()
    for x in [8, 22]:
        rect(img, x-3, 23, x+3, 27, (255,255,255,255))
        rect(img, x-3, 27, x+3, 28, (60,140,200,255))
    return img

def feet_sandals():
    img = new()
    for x in [8, 22]:
        rect(img, x-2, 25, x+2, 27, (140,90,60,255))
...
def feet_skates():
    img = new()
    for x in [8, 22]:
        rect(img, x-3, 24, x+3, 26, (60,140,200,255))
        rect(img, x-3, 27, x+3, 28, (160,160,180,255))
    return img

def feet_paws():
    img = new()
    for x in [8, 22]:
        rect(img, x-3, 23, x+3, 27, (255,180,200,255))
...
def feet_slippers():
    img = new()
    for x in [8, 22]:
        rect(img, x-3, 24, x+3, 28, (255,180,200,255))
    return img

def feet_glow():
    img = new()
    for x in [8, 22]:
        rect(img, x-3, 24, x+3, 28, (255,255,200,255))
        rect(img, x-2, 26, x+2, 27, (255,200,80,255))
    return img
...
def feet_stripe():
    ...
    for x in [8, 22]:
        rect(img, x-2, 22, x+2, 28, (255,255,255,255))
        for y in [23, 25, 27]:
            hline(img, x-2, x+2, y, (60,140,200,255))
    return img

def feet_high():
    img = new()
    for x in [8, 22]:
        rect(img, x-2, 18, x+2, 28, (255,80,80,255))
        rect(img, x-2, 18, x+2, 19, (200,40,60,255))
    return img

...
# ---------- CAT ----------
# Body color = white (CSS hue/saturate gives variants)
CAT_FUR = (235, 220, 200, 255)
CAT_FUR_D = (200, 180, 160, 255)


def cat_body(img):
    # 뾰족한 V 귀 (좌우)
    for i in range(4):
        hline(img, 6+i, 7-i, 6+i, OUT)  # left
        hline(img, 24+i, 25-i, 6+i, OUT)  # right
    # ear interior (lighter)
    px(img, 6, 7, CAT_FUR_D); px(img, 7, 8, CAT_FUR_D)
    px(img, 25, 7, CAT_FUR_D); px(img, 24, 8, CAT_FUR_D)
    # head (둥근 사각, 슬림하게)
    hline(img, 9, 22, 8, OUT)
    vline(img, 8, 9, 16, OUT)
    vline(img, 23, 9, 16, OUT)
    hline(img, 9, 22, 17, OUT)
    rect(img, 9, 9, 22, 16, CAT_FUR)
    rect(img, 12, 13, 19, 16, BELLY)
    # body (slim 사각)
    vline(img, 9, 18, 24, OUT)
    vline(img, 22, 18, 24, OUT)
    hline(img, 10, 21, 25, OUT)
    rect(img, 10, 18, 21, 24, CAT_FUR)
...
# ---------- RABBIT ----------
RAB_FUR = (240, 235, 225, 255)
RAB_FUR_D = (210, 200, 190, 255)


def rabbit_body(img):
    # 길쭉한 두 귀 (위로)
    rect(img, 9, 1, 11, 9, OUT)
    rect(img, 10, 2, 10, 8, RAB_FUR)
    rect(img, 20, 1, 22, 9, OUT)
    rect(img, 21, 2, 21, 8, RAB_FUR)
    # 귀 안 핑크
...
SPECIES = {
    'cat':     cat_body,
    'rabbit':  rabbit_body,
    'hamster': hamster_body,
}


def make_mood(species, mood):
    img = new()
    body_fn = SPECIES[species]
    body_fn(img)
    if mood == 'idle':
...
def main():
    for sp in SPECIES:
        out_dir = os.path.join(BASE, sp)
        os.makedirs(out_dir, exist_ok=True)
        for mood in ['idle', 'happy', 'eating', 'sad', 'sleeping']:
            img = make_mood(sp, mood)
